...
class Cfp::SessionsControllerTest < ActionController::TestCase
  ...
  test 'submitter can login' do
    post :create, params: { conference_acronym: @conference.acronym, user: { email: @submitter.email, password: 'frab123' } }
  ...
  test 'cannot login with wrong password' do
    post :create, params: { conference_acronym: @conference.acronym, user: { email: @submitter.email, password: 'wrong' } }
    assert_nil assigns(:current_user)
    assert_response :success
  end
